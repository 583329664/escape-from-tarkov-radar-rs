...
use domain::models::player::{Player, PlayerType};
use eframe::{
    egui::{self, Label},
    emath::{Pos2, Vec2},
...
impl eframe::App for Radar {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().frame(frame).show(ctx, |ui| {
            ...
            let local_player_res = players
                .iter()
                .find(|p| p.player_type.clone() == PlayerType::Local);
            ...
            for player in players.iter() {
                ...
                let color = match (player.player_type.clone(), player.location) {
                    (PlayerType::Local, _) => Color32::GREEN,
                    (PlayerType::Dead, _) => Color32::BLACK,
                    (_, location) if local_player.location.z - location.z > 5.0 => {
                        Color32::LIGHT_RED
                    }
                    (_, location) if local_player.location.z - location.z < -5.0 => {
                ...
                if player.player_type == PlayerType::Dead && player.last_aggressor.is_some() {
                    ui.heading(format!(
                        "{} was killed by {}",
                        player.name,
